...
describe('RedisRateLimitStore (live Redis, two clients; skipped without Redis)', () => {
  ...
  it('guards built on two services (two API processes) share the login budget', async () => {
    ...
    expect(await guardA.canActivate(guardContext('192.0.2.77', email))).toBe(true);
    expect(await guardB.canActivate(guardContext('192.0.2.77', email))).toBe(true);
    const headers: Record<string, string> = {};
    const err = await guardA.canActivate(guardContext('192.0.2.77', email, headers)).catch((e: unknown) => e);
    expect((err as HttpException).getStatus()).toBe(429);
