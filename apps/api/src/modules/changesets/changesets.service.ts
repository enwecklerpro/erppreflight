...
export class ChangeSetsService {
  ...
  async simulate(organizationId: string, projectId: string, id: string) {
    ...
    // Fetch baseline findings if available
    ...
    if (changeset.baseline_analysis_id) {
      ...
      const findingsRes = await this.db.query(
        `SELECT * FROM findings WHERE project_id = $1 AND organization_id = $2 LIMIT 50`,
    ...
    for (const change of proposedChanges) {
      if (change.type === 'REMOVE_CUSTOM_FIELD') {
        ...
        // Check if baseline had API_V2 deprecation warning
        const matchingBaseline = baselineFindings.find((f) => f.rule_id?.includes('API'));
        if (matchingBaseline) {
          resolvedFindings.push({
            id: matchingBaseline.id,
            ruleId: matchingBaseline.rule_id,
            title: matchingBaseline.title,
            reason: `Migrating to OData v4 resolves deprecated endpoint warning.`,
          });
        }
      } else {
        // Generic change
    ...
    const riskDelta = newFindings.filter((f) => f.severity === 'CRITICAL' || f.severity === 'BLOCKER').length > 0
      ? 'INCREASED'
    ...
    const simulationResult = {
      ...
      requiredTests,
      riskDelta,
      verdict: newFindings.some((f) => f.severity === 'BLOCKER')
        ? 'BLOCKED'
        : newFindings.some((f) => f.severity === 'CRITICAL')
