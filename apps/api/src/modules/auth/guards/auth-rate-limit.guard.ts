 ...
 * Express `trust proxy` (TRUST_PROXY = real proxy hop count) behind a reverse proxy so the
 * client address is real; the API port must only be reachable through that proxy.
...
export class AuthRateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Same client address as the organization IP allowlist (clientIpOf: req.ip under the
    // TRUST_PROXY hop count, IPv4-mapped IPv6 folded to IPv4), so one TRUST_PROXY setting
    // governs both controls.
