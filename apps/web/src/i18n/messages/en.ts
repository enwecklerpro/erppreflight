...
export const en = {
  ...
  legal: {
    ...
    cookies: {
      ...
      necessary: 'Necessary',
      items: [
        { name: 'erppreflight_session', purpose: 'Keeps you signed in (set by the API, HTTP-only, not readable by scripts).', duration: '7 days' },
        { name: 'erp_csrf', purpose: 'Protects your session against cross-site request forgery (set by the API, contains no credential).', duration: '7 days' },
        ...
        { name: 'erppreflight_tenant_id, erppreflight_session_epoch', purpose: 'Browser storage for the active organization and a sign-in/sign-out signal between tabs (no credentials).', duration: 'Until sign-out' },
      ],
