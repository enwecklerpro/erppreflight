...
export function ConfidenceBadge({
  ...
  const t = useT();
  const key = (confidenceConfig[confidence] ? confidence : 'UNKNOWN') as ConfidenceClass;
  const conf = confidenceConfig[key];
  const label = t(`app.findings.badges.confidence.${key}`);
  ...
  return (
    <span
      role="status"
      aria-label={t('app.findings.badges.confidenceAria', { label, score: displayScore })}
