...
from src.core.registry import EngineRegistry, register_engine  # noqa: E402
...
# =============================================================================
# 10. Remediation Regression Tests (Reviewer 2 & Challenger 2 Defect Closure)
# =============================================================================

from src.engines.api_change import ApiChangeEngine, ClientIntegration  # noqa: E402


@pytest.mark.asyncio
async def test_remediation_1_swagger2_missing_definitions():
    """Fix 1: Verifies Swagger 2.0 without 'definitions' key parses cleanly without crashing."""
    setup_domain3_engines()
    eng = ApiChangeEngine()
    doc = {"swagger": "2.0", "paths": {}}
    schema = eng._parse_openapi(doc, "{}", "test_swagger2.json", "fake_hash")
    assert schema.entities == {}


@pytest.mark.asyncio
async def test_remediation_2_optional_to_required_parameter_transition():
    """Fix 2: Verifies transitioning an existing parameter from optional to required emits API_BREAKING_REQUIRED_PARAM_ADDED."""
    setup_domain3_engines()
    base = {
        "openapi": "3.0.0",
        "paths": {
    ...
    req = AnalysisRequest(
        ...
        tenant_id="22222222-0001-0001-0001-000000000001",
        project_id="33333333-0001-0001-000000000001",
        engine_type=EngineType.API_CHANGE_GUARD,
        configuration={"baseline": base, "candidate": cand},
    )
    resp = await EngineRunner.execute(req)
    assert resp.status == AnalysisStatus.COMPLETED
    assert any(f.rule_id == "API_BREAKING_REQUIRED_PARAM_ADDED" for f in resp.findings)


@pytest.mark.asyncio
async def test_remediation_3_incompatible_parameter_type_mutation():
    """Fix 3: Verifies changing a parameter from integer to boolean emits API_BREAKING_TYPE_CHANGED."""
    setup_domain3_engines()
    base = {
        "openapi": "3.0.0",
        "paths": {
    ...
    cand = {
        ...
        "paths": {
            "/items": {
                "get": {
                    "parameters": [{"name": "limit", "in": "query", "schema": {"type": "boolean"}}]
                }
            }
        },
    }
    ...
    resp = await EngineRunner.execute(req)
    assert resp.status == AnalysisStatus.COMPLETED
    assert any(f.rule_id == "API_BREAKING_TYPE_CHANGED" for f in resp.findings)


@pytest.mark.asyncio
async def test_remediation_4_number_to_string_incompatible_type():
    """Fix 4: Verifies mutating a number property/parameter to string emits API_BREAKING_TYPE_CHANGED."""
    setup_domain3_engines()
    eng = ApiChangeEngine()
    assert eng._is_incompatible_type_change("number", "string") is True
    ...
    base = {
        ...
        "paths": {
            "/items": {
                "get": {
                    "parameters": [{"name": "price", "in": "query", "schema": {"type": "number"}}]
                }
            }
        },
    }
    cand = {
        ...
        "paths": {
            "/items": {
                "get": {
                    "parameters": [{"name": "price", "in": "query", "schema": {"type": "string"}}]
                }
            }
        },
    }
...
@pytest.mark.asyncio
async def test_remediation_5_odata_edmx_clark_notated_deprecation():
    """Fix 5: Verifies that OData EDMX containing Clark-notated sap:deprecated='true' is detected."""
    setup_domain3_engines()
    base_xml = (
        '<?xml version="1.0" encoding="utf-8"?>\n'
    ...
    req = AnalysisRequest(
        ...
        project_id="33333333-0001-0001-000000000001",
        engine_type=EngineType.API_CHANGE_GUARD,
        configuration={"baseline": base_xml, "candidate": cand_xml},
    )
    resp = await EngineRunner.execute(req)
    assert resp.status == AnalysisStatus.COMPLETED
    assert any(f.rule_id == "API_DEPRECATION_WARNING" for f in resp.findings)


@pytest.mark.asyncio
async def test_remediation_6_non_breaking_operation_count_telemetry():
    """Fix 6: Verifies adding an operation to an existing route increments nonBreakingChangesCount in metrics."""
    setup_domain3_engines()
    base = {
        "openapi": "3.0.0",
    ...
    resp = await EngineRunner.execute(req)
    assert resp.status == AnalysisStatus.COMPLETED
    assert any(f.rule_id == "API_NON_BREAKING_OPERATION_ADDED" for f in resp.findings)
    assert resp.metrics.additional_metrics.get("nonBreakingChangesCount") == 1
...
async def test_remediation_7_bundled_payload_diagnoses_candidate_missing():
    ...
    req = AnalysisRequest(
        job_id="11111111-9999-0001-0001-000000000006",
        tenant_id="22222222-0001-0001-0001-000000000001",
        project_id="33333333-0001-0001-000000000001",
        engine_type=EngineType.API_CHANGE_GUARD,
        raw_content=bundle_with_baseline_only,
    )
    resp = await EngineRunner.execute(req)
    assert resp.status == AnalysisStatus.FAILED
    assert resp.findings[0].rule_id == "API_INSUFFICIENT_INPUT"
...
@pytest.mark.asyncio
async def test_remediation_8_consumer_operation_not_overmatched_by_endpoint_fallback():
    """Fix 8: Verifies read-only consumer with route op filter is not overmatched on operation removal."""
    setup_domain3_engines()
...
@pytest.mark.asyncio
async def test_remediation_9_entity_name_prefix_stripping_no_corruption():
    """Fix 9: Verifies that removeprefix('a_') matches correctly without corrupting internal substrings."""
    setup_domain3_engines()
    engine = ApiChangeEngine()
    integ = ClientIntegration(
        integration_id="CRM_SYNC",
        consumed_fields={"data_area": ["Field1"]},
    )
    # Target entity is "A_Data_Area" - should match "data_area" via removeprefix("a_")
    impacted = engine._cross_reference_field("A_Data_Area", "Field1", integrations=[integ])
    assert "CRM_SYNC" in impacted
