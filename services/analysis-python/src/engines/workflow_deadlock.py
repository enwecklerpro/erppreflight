...
class WorkflowStuckEngine(BaseEngine):
    ...
    def _evaluate_workflow_rules(
        ...
        for log_item in context.logs:
            logs_by_id.setdefault(log_item.wi_id, []).append(log_item)
        ...
        for wi_id, header in headers_by_id.items():
            ...
            has_error_log = any(
                log_item.retcode > 0 or log_item.exception is not None or (log_item.msgid and "ERROR" in log_item.msgid.upper())
            ...
            if is_error_status or (is_bg_type and has_error_log):
                ...
                for log_item in logs:
                    if log_item.exception:
                        exc_str = log_item.exception
                    if log_item.retcode > 0:
                        retcode_val = log_item.retcode
                    if log_item.method:
                        method_name = log_item.method
    ...
    def _merge_dict_into_context(
        ...
        if isinstance(links_data, list):
            for idx, link_dict in enumerate(links_data):
                if isinstance(link_dict, dict):
                    norm_l = {k.lower(): v for k, v in link_dict.items()}
