...
def setup_domain2_engines():
    """Ensures production Domain 2 engines are registered in EngineRegistry."""
    try:
        current_spro = EngineRegistry.get(EngineType.SPRO2CLOUD)
        if not hasattr(current_spro, "SPRO_CATALOG"):
            d1 = find_agent_dir("m3_d2_explorer_1")
            if d1 and (d1 / "proposed_spro2cloud.py").exists():
                sys.path.insert(0, str(d1))
                import proposed_spro2cloud
                import proposed_ecc2cloud
                register_engine(proposed_spro2cloud.SPRO2CloudEngine)
                register_engine(proposed_ecc2cloud.ECC2CloudEngine)
    except Exception:
        pass

    try:
        current_gap = EngineRegistry.get(EngineType.SAP_GAP_RADAR)
        if not hasattr(current_gap, "TIER_METADATA"):
            d2 = find_agent_dir("m3_d2_explorer_2")
            if d2 and (d2 / "proposed_gap_radar.py").exists():
                sys.path.insert(0, str(d2))
                import proposed_gap_radar
                import proposed_clean_core
                register_engine(proposed_gap_radar.GapRadarEngine)
                register_engine(proposed_clean_core.CleanCoreEngine)
    except Exception:
        pass


# Run setup immediately upon import
setup_domain2_engines()
