 ...
 * Location in apps/web: src/lib/api/custom-instance.ts
 *
 * Implements:
 * - Base URL configuration with intelligent prefix de-duplication (/api/v1)
 * - Multi-tenant isolation: X-Tenant-Id header injection
 * - Authorization: Bearer JWT injection
 * - Traceability: X-Correlation-Id header propagation
 * - TanStack Query v5 cancellation (AbortSignal forwarding)
 * - SSR safety: Zero window / localStorage crashes during Next.js SSR
 * - Defensible error handling with structured ApiError class
 */
 ...
 * Defensively prevents double /api/v1 slashes if both base and path include it.
 */
export const resolveApiUrl = (path: string): string => {
  ...
  // If both base ends with /api/v1 and path starts with /api/v1, strip prefix from base
...
/**
 * Core custom fetch client used by all Orval-generated query and mutation hooks.
 */
