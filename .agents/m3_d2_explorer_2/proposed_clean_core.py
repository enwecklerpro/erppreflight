...
class CleanCoreEngine(BaseEngine):
    ...
    def evaluate(cls, abap_code: str) -> Dict[str, Any]:
        ...
        for line_no, raw_line in enumerate(lines, start=1):
            ...
            line = code_part.upper()

            # Skip empty lines or pure comment lines starting with '*'
            if not line or raw_line.strip().startswith("*"):
                continue
            ...
            for stmt, meta in OBSOLETE_STATEMENTS_MAP.items():
                ...
                if stmt == "CALL 'SYSTEM'":
                    ...
                    if line.startswith("CALL TRANSACTION ") or line.startswith("CALL TRANSACTION:"):
                        is_match = True
                elif stmt == "EXEC SQL":
                    if line.startswith("EXEC SQL") or "EXEC SQL" in line:
                        is_match = True
                else:
                    if line.startswith(f"{stmt} ") or line.startswith(f"{stmt}:") or line == stmt:
                        is_match = True
                ...
                if is_match:
                    ...
                    findings.append({
                        ...
                        "line": line_no,
                        "statement": stmt,
                        "full_statement": code_part,
                        "remediation": meta["remediation"],
                        "confidence": "VERIFIED",
                    })

            # 3. Check unreleased function module calls
            m_fn = cls.FUNCTION_CALL_REGEX.search(line)
            if m_fn:
