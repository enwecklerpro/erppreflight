...
from src.parsers.safe_xml import LineElement, SafeXmlParser
...
class ApiChangeEngine(BaseEngine):
    ...
    async def analyze(self, request: AnalysisRequest) -> AnalysisResponse:
        ...
        # Status is COMPLETED unless a BLOCKER finding was emitted
        has_blocker = any(f.severity == Severity.BLOCKER for f in classified_findings)
    ...
    def _parse_request_inputs(
        ...
        if not baseline_raw and request.raw_content and request.raw_content.strip():
    ...
    def _diff_schemas(
        ...
        for ep_path, base_ep in baseline.endpoints.items():
            if ep_path not in candidate.endpoints:
                ...
                op_findings, op_evals, op_breaking = self._diff_operations(
                    ep_path=ep_path,
                    base_ep=base_ep,
                    cand_ep=cand_ep,
                    baseline=baseline,
                ...
                for param_name, cand_param in cand_op.parameters.items():
                    ...
                    if cand_param.required and param_name not in base_op.parameters:
                        ...
                        finding = self._build_finding(
                            ...
                            description=(
                                f"Required {cand_param.in_location} parameter '{param_name}' was added to '{method} {ep_path}'. "
