...
class TransportDependencyEngine(BaseEngine):
    ...
    def _parse_csv_content(self, csv_text: str, data: CTSNormalizedData) -> None:
        ...
        for line_idx, row in enumerate(raw_rows[start_idx:], start=start_idx + 1):
            ...
            obj_name = get_col("OBJ_NAME", 2)
            tbl_name = get_col("TABLENAME", 2)
            ...
            if "TRFUNCTION" in col_map or record_type == "E070":
                ...
                data.headers[tr] = E070Record(
                    trkorr=tr,
                    trfunction=get_col("TRFUNCTION", 1).upper() or "K",
                ...
                if tbl:
                    ...
                    if tr not in data.headers:
                        data.headers[tr] = E070Record(trkorr=tr, trfunction="W", line_number=line_idx, snippet=snip)
            else:
                # E071 Object row
    ...
    def _run_deterministic_rules(cls, data: CTSNormalizedData) -> Dict[str, Any]:
        ...
        # ----------------------------------------------------------------------
        # Cycle Detection via Tarjan / DFS
    ...
    async def analyze(self, request: AnalysisRequest) -> AnalysisResponse:
        ...
        for raw_f in eval_result["findings"]:
            ...
            if data.raw_content:
                l, c, s = _locate_line_in_text(data.raw_content, obj_name)
                line_no = l
                col_no = c
                snip = s
                ...
                l, c, s = _locate_line_in_text(data.raw_content, first_tr)
                line_no = l
                col_no = c
                snip = s
